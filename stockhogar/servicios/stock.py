...
from ..rutas.hogares import _usuario_tiene_permiso
...
def hogar_actual_con_permiso(db, session, nivel_requerido=None):
    ...
    if not _usuario_tiene_permiso(db, hogar_id, usuario_id, nivel_requerido):
        return None
    return hogar_id


def _resolver_hogar_id_por_defecto(db, session):
    """Obtiene la hogar_id de sesión o, si no hay, la primera lista del usuario."""
    hogar_id = session.get("hogar_actual_id")
    if hogar_id:
        return hogar_id
...
def revisar_stock_bajo(db, producto_id, hogar_id=None):
    """Mantiene la lista de la compra en sincronia con el stock del producto (por lista)."""
    try:
        from flask import session

        if hogar_id is None:
            hogar_id = _resolver_hogar_id_por_defecto(db, session)
        ...
        pendiente = db.execute(
            "SELECT id FROM articulos_compra WHERE producto_id = ? AND origen = 'auto' AND activo = 1 AND hogar_id = ?",
        ...
        if cantidad <= stock_minimo:
            if pendiente is None:
                ...
                completado = db.execute(
                    "SELECT id FROM articulos_compra WHERE producto_id = ? AND activo = 0 AND hogar_id = ?",
                    (producto_id, hogar_id),
                ).fetchone()
                if completado is not None:
                    db.execute(
                        "UPDATE articulos_compra SET activo = 1, origen = 'auto', nombre = ?, unidad = ?, "
                    ...
                    db.execute(
                        ...
                        "(hogar_id, producto_id, nombre, unidad, categoria, icono, origen, fecha_creacion) "
                        "VALUES (?, ?, ?, ?, ?, ?, 'auto', ?)",
                        (hogar_id, producto_id, nombre, unidad, categoria, icono, ahora()),
                    )
...
def registrar_movimiento(db, producto_id, hogar_id, delta, cantidad_resultante, origen="ajuste"):
    ...
    db.execute(
        """INSERT INTO movimientos_stock
           (producto_id, hogar_id, usuario_id, delta, cantidad_resultante, origen, fecha)
           VALUES (?, ?, ?, ?, ?, ?, ?)""",
        (producto_id, hogar_id, usuario_id, delta, cantidad_resultante, origen, ahora()),
    )


def sumar_stock(db, producto_id, cantidad_a_sumar, hogar_id=None):
    ...
    db.execute(
        """UPDATE stock_hogar SET cantidad = MAX(0, cantidad + ?), fecha_actualizacion = ?
           WHERE hogar_id = ? AND producto_id = ?""",
        (cantidad_a_sumar, ahora(), hogar_id, producto_id)
    )

    nueva_cantidad = max(0, actual["cantidad"] + cantidad_a_sumar)
    registrar_movimiento(db, producto_id, hogar_id, nueva_cantidad - actual["cantidad"], nueva_cantidad)
...
def crear_producto_nuevo(
    ...
    icono=None, hogar_id=None,
):
    """Crea producto en catálogo y registra stock en stock_hogar."""
    from flask import session
    ...
    if hogar_id:
        try:
            ...
            logger.debug(f"[crear_producto_nuevo] Error en stock_hogar: {e}")
