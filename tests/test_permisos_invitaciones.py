...
class PermisosInvitacionesTests(unittest.TestCase):
    def setUp(self):
        ...
        self.propietario_id, self.lista_id, self.client_propietario = self._crear_usuario_con_lista("propietario")
    ...
    def tearDown(self):
        with self.app.app_context():
            db = get_db()
            db.execute(
                "DELETE FROM invitaciones_lista WHERE lista_id = ?", (self.lista_id,)
            )
    ...
    def test_generar_enlace_compartible_solo_propietario(self):
        resp = self.client_invitado.post(f"/api/listas/{self.lista_id}/enlace-compartible")
        self.assertEqual(resp.status_code, 403, resp.get_data(as_text=True))

        resp = self.client_propietario.post(f"/api/listas/{self.lista_id}/enlace-compartible")
    ...
    def test_aceptar_invitacion_da_acceso_y_la_invalida(self):
        ...
        resp = self.client_invitado.post(f"/api/listas/aceptar-invitacion/{codigo}")
        self.assertEqual(resp.status_code, 200, resp.get_data(as_text=True))
        ...
        with self.app.app_context():
            ...
            permiso = db.execute(
                ...
                (self.lista_id, self.invitado_id),
            ).fetchone()
        self.assertIsNotNone(permiso)

        resp_repetido = self.client_invitado.post(f"/api/listas/aceptar-invitacion/{codigo}")
    ...
    def test_actualizar_y_revocar_permiso(self):
        with self.app.app_context():
            ...
            db.execute(
                ...
                (self.lista_id, self.invitado_id, ahora()),
            )
            db.commit()

        resp = self.client_propietario.patch(
            f"/api/listas/{self.lista_id}/permisos/{self.invitado_id}", json={"nivel": "editar"}
        )
        self.assertEqual(resp.status_code, 200, resp.get_data(as_text=True))
        ...
        resp = self.client_propietario.delete(f"/api/listas/{self.lista_id}/permisos/{self.invitado_id}")
        self.assertEqual(resp.status_code, 200, resp.get_data(as_text=True))

        with self.app.app_context():
            db = get_db()
            fila = db.execute(
                "SELECT 1 FROM permisos_lista WHERE lista_id = ? AND usuario_id = ?",
                (self.lista_id, self.invitado_id),
            ).fetchone()
    ...
    def test_no_se_puede_modificar_permiso_del_propietario(self):
        resp = self.client_propietario.patch(
            f"/api/listas/{self.lista_id}/permisos/{self.propietario_id}", json={"nivel": "ver"}
        )
